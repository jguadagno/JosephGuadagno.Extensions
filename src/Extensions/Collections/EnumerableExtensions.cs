...
namespace JosephGuadagno.Extensions.Collections
{
    /// <summary>
    /// Extensions for the <see cref="Enumerable"/> object.
    /// </summary>
    public static class EnumerableExtensions
    {
            ...
            Func<TSource, IEnumerable<TSource>> getChildrenFunction)
        {
            ...
            var flattenedList = enumerable.Where(selectorFunction);

            // Go through the input enumerable looking for children,
            // and add those if we have them
            return enumerable.Aggregate(flattenedList,
                (current, element) =>
                        current.Concat(getChildrenFunction(element).Map(selectorFunction, getChildrenFunction)));
        }
